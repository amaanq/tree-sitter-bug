...
static inline bool string_eq(String *self, String *other) {
    ...
    return memcmp(self->contents, other->contents, self->size) == 0;
}
...
static void deserialize(Scanner *scanner, const char *buffer, unsigned length) {
    ...
    for (uint32_t i = open_heredoc_count; i < scanner->heredocs.size; i++) {
        printf("HEREDOC[%d:%d]: ", i, scanner->heredocs.size);
        for (uint32_t j = 0; j < scanner->heredocs.contents[i].word.size; j++) {
            printf("%d:", scanner->heredocs.contents[i].word.contents[j]);
        }
        printf("\n");
    }
...
static inline bool scan_nowdoc_string(Scanner *scanner, TSLexer *lexer, const bool *valid_symbols) {
    ...
    printf("NOWDOC_STRING[%d]: ", heredoc_tag.size);
    for (uint32_t i = 0; i < heredoc_tag.size; i++) {
        printf("%d:", heredoc_tag.contents[i]);
    }
    printf("\n");
